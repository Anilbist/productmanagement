...
<?php include 'db.php';
  ...
  //    	$dst="product_image/".$v2.$fna;
     	$query =$connect->prepare("SELECT pimage FROM pro_image where Sn =?");
    	$query->execute([$Pro_sn]);
    	$row = $query->fetch(PDO::FETCH_OBJ);
    	unlink($row->pimage);
     	move_uploaded_file($_FILES['p_image']['tmp_name'][i],$dst);
		$Query= $connect->prepare("UPDATE pro_image SET proid=? pimage=? WHERE Sn ='$Pro_sn'");
		$Query->execute([$last_id,$dst]);
	}
}
	else{
		$Query= $connect->prepare("UPDATE prod SET pname =?,pdis = ?,pprice=? WHERE Sn ='$Pro_sn'");
